...
//       thumbsPerRow_ = NSIntegerMin;
       //d3H: Doan nay sua lai...
//      thumbsPerRow_ = NSIntegerMin;//TODO: sua lai thanh NSIntegerMin // Forces caluation because on view size.
//        thumbSize_ = CGSizeMake(135 , 135);
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)
        {
      thumbSize_ = CGSizeMake(128.4 , 128.4);
        }
        else{
...
//       self.frame=CGRectMake(0, 88, 768, 936);
   }
   return self;
}
...
//    visibleBounds=CGRectMake(0, 88, 768, 936);
   ...
   if (visibleWidth - itemsPerRow * thumbSize_.width < minimumSpace) {
    ...
    int initialY = [[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad?0:0;
   ...
   int x = spaceWidth;
   int y = spaceHeight + (topRow * rowHeight)+initialY;
