...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    {
        NSDictionary *userInfo = [[info objectForKey:@"userInfo"] retain];
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
                {
                    NSDictionary *userInfo = [[[downloadInfo objectAtIndex:uidx] objectForKey:@"userInfo"] retain];
                    if ([userInfo isKindOfClass:NSNull.class])
                    {
                        userInfo = nil;
                    }
                    // Fixing crashing on ios7.0 or later
                    //objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:userInfo:), self, [image retain], downloader.url, userInfo);
