...
// Private Properties
@property(nonatomic,readwrite)	NSUInteger _currentPageIndex;
...
- (void)viewDidLoad {
    ...
    if (version >= 7.0) {
        ...
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
            [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"slideSHOW_titlebar_bk"]
	...
	_pagingScrollView.backgroundColor = [SettingView sharedCache].appBackgroundColor;
    ...
    if ([[UIToolbar class] respondsToSelector:@selector(appearance)]) {
        [_toolbar setBackgroundImage:[UIImage imageNamed:@"slideSHOW_titlebar_bk"]
    ...
    NSLog(@"background-rect: %@", NSStringFromCGRect(_toolbar.frame));
//    [_toolbar setBackgroundImage:[UIImage imageNamed:@"slideSHOW_titlebar_bk"] forToolbarPosition:UIBarPositionTop barMetrics:UIBarMetricsDefault];
...
//    _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(actionButtonPressed:)];
    _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"shareBTN_iPhone.png"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];

    if (version >= 7.0) {
        _toolbar.tintColor = [SettingView sharedCache].colorBarButton;
    }
    else {
        _previousButton.tintColor = [SettingView sharedCache].colorBarButton;
        _nextButton.tintColor = [SettingView sharedCache].colorBarButton;
        _actionButton.tintColor = [SettingView sharedCache].colorBarButton;
    }
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        ...
        //        UIImageView *_imageView = [[UIImageView alloc] initWithFrame:CGRectMake(-80, 0, 1024, 130)];
        
        //        [_imageView setImage:[UIImage imageNamed:@"slideSHOW_titlebar_bk"]];
        
        
        UIView *_view1 = [[UIView alloc] initWithFrame:CGRectMake(0, 0,768, 88)];
        _view1.backgroundColor = [UIColor clearColor];//[UIColor colorWithPatternImage:[UIImage imageNamed:@"slideSHOW_titlebar_bk"]];
        ...
        if (version >= 5.0) {
            UIImage *backgroundImage = [UIImage imageNamed:@"slideSHOW_titlebar_bk"];
            [self.navigationController.navigationBar setBackgroundImage:backgroundImage forBarMetrics:UIBarMetricsDefault];
        }
        else
        {
            // UIImage *backgroundImage = [UIImage imageNamed:@"image.png"];
            NSString *barBgPath = [[NSBundle mainBundle] pathForResource:@"slideSHOW_titlebar_bk" ofType:@"png"];
...
- (void)setNavibationImage:(NSString*)imgBg {
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
    ...
    float version = [[[UIDevice currentDevice] systemVersion] floatValue];
    if (version >= 7.0) {
        [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:imgBg]
                                                     forBarPosition:UIBarPositionAny
                                                         barMetrics:UIBarMetricsDefault];
        
    }
    else if (version >= 5.0) {
        UIImage *backgroundImage = [UIImage imageNamed:imgBg];
        [self.navigationController.navigationBar setBackgroundImage:backgroundImage forBarMetrics:UIBarMetricsDefault];
    }
    else
    {
        // UIImage *backgroundImage = [UIImage imageNamed:@"image.png"];
        NSString *barBgPath = [[NSBundle mainBundle] pathForResource:imgBg ofType:@"png"];
        [self.navigationController.navigationBar.layer setContents:(id)[UIImage imageWithContentsOfFile: barBgPath].CGImage];
    }
    //d3H: set navigation bar clear color
    self.navigationController.navigationBar.backgroundColor = [UIColor clearColor];
    [self.view bringSubviewToFront:self.navigationController.navigationBar];
}

- (void) setToolbarImage:(NSString*)imgBg
{
...
-(void)custom_back {
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        UIView *_view1 = [[[UIView alloc] initWithFrame:CGRectMake(0, 0,buttonImage.size.width+50, 130)] autorelease];
        _view1.backgroundColor = [UIColor clearColor];
        button.frame = CGRectMake(10, 28, 50, 34);
        [button setImage:[[UIImage imageNamed:@"BACKbtn_ipad.png"] imageWithColor:[SettingView sharedCache].colorBarButton] forState:UIControlStateNormal];
        [_view1 addSubview:button];
        
        UIBarButtonItem *customBarItem = [[UIBarButtonItem alloc] initWithCustomView:_view1];
        customBarItem.tintColor = [SettingView sharedCache].colorBarButton;
...
//    button.tintColor = [SettingView sharedCache].colorBarButton;
}
-(void)back {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self setNavibationImage:@"slideSHOW_titlebar_bk"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self setNavibationImage:@"categoryHeader"];
    // Status bar
    if (self.wantsFullScreenLayout && UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    UILabel *lbl = (UILabel *)[self.navigationItem.titleView viewWithTag:1002];
    if (lbl) {
        lbl.text = self.title;
    }
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    MWCaptionView *captionView = [self captionViewForPhotoAtIndex:_currentPageIndex];
    captionView.frame = [self frameForCaptionView:captionView atIndex:_currentPageIndex];
    [_pagingScrollView addSubview:captionView];
    [captionView addBackgroundImage];
    page.captionView = captionView;
    NSLog(@"captionview frame: %@", NSStringFromCGRect(captionView.frame));
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
        ...
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
            //            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,20,650,44)];
            //            titleLabel.backgroundColor = [UIColor clearColor];
            //            titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:34.0];
            //            titleLabel.textColor=[UIColor whiteColor];
            //            titleLabel.textAlignment = UITextAlignmentCenter;
            
            UILabel *titleLabel = (UILabel *)[self.navigationItem.titleView viewWithTag:1002];
            titleLabel.text = self.title;
            titleLabel.textColor = [SettingView sharedCache].textColor;
        }
...
- (void)gotoPreviousPage
{
    ...
    [appdel sendDataTogoogleAnalytics:[NSString stringWithFormat:@"%@",DetailPhotoviewPreviousButtonClicked]];
...
- (void)gotoNextPage
{
    ...
    AppDelegate *appdel=(AppDelegate*)[UIApplication sharedApplication].delegate;
    [appdel sendDataTogoogleAnalytics:[NSString stringWithFormat:@"%@",DetailPhotoviewNextButtonClicked]];
...
- (void)actionButtonPressed:(id)sender {
    ...
    AppDelegate *appdel=(AppDelegate*)[UIApplication sharedApplication].delegate;
    [appdel sendDataTogoogleAnalytics:[NSString stringWithFormat:@"%@",DetailPhotoviewActionButtonClicked]];
    ...
    if (_actionsSheet) {
        ...
        //D3H: show default action sheet.
        id <MWPhoto> currentPhoto = [self photoAtIndex:_currentPageIndex];
        
        NSString *textObject = [NSString stringWithFormat:@"%@ - %@",[currentPhoto caption],kShareAppName];
        NSString *urlString = kShareLinkApp;        
        NSURL *url = [NSURL URLWithString:urlString];

//        NSString *textObject = @"Some text ";
        
        NSArray *activityItems = [NSArray arrayWithObjects:textObject, url,
                                  [currentPhoto underlyingImage],
                                  nil];
...
//                [self.activityPopover presentPopoverFromRect:btn.frame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
//            }
//            else
//            {
    ...
    if (actionSheet == _actionsSheet) {
        ...
        if (buttonIndex != actionSheet.cancelButtonIndex) {
            if (buttonIndex == actionSheet.firstOtherButtonIndex) {
                AppDelegate *appdel=(AppDelegate*)[UIApplication sharedApplication].delegate;
                [appdel sendDataTogoogleAnalytics:[NSString stringWithFormat:@"%@",DetailPhotoviewActionSaveButtonClicked]];
                [self savePhoto]; return;
            } else if (buttonIndex == actionSheet.firstOtherButtonIndex + 1) {
                AppDelegate *appdel=(AppDelegate*)[UIApplication sharedApplication].delegate;
                
                [appdel sendDataTogoogleAnalytics:[NSString stringWithFormat:@"%@",DetailPhotoviewActionCopyButtonClicked]];
                [self copyPhoto]; return;
            } else if (buttonIndex == actionSheet.firstOtherButtonIndex + 2) {
                AppDelegate *appdel=(AppDelegate*)[UIApplication sharedApplication].delegate;
                
                [appdel sendDataTogoogleAnalytics:[NSString stringWithFormat:@"%@",DetailPhotoviewActionEmailButtonClicked]];
...
- (void)actuallyEmailPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [emailer setSubject:NSLocalizedString(@"PortfolioApp by Kapova.com", nil)];
